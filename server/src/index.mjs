...
// Wajibkan autentikasi untuk koneksi Socket.IO (fix Strix vuln-0001):
// tanpa token JWT valid -> koneksi ditolak, PII tidak bocor.
io.use((socket, next) => {
...
// Status pipeline yang dirender sebagai kolom di KanbanScreen (urutan kolom).
// data_masuk (diurus lewat tab Scan) & absen_ambil_customer (tab Self Pickup)
// sengaja TIDAK masuk — query kanban wajib dibatasi status ini.
const KANBAN_STATUSES = [
  'absen_buyback', 'absen_gojek', 'mencari_driver', 'driver_sampai_kios',
  'selesai', 'retur', 'cancel', 'dikirim_ke_gudang', 'diterima_gudang',
];
...
app.get('/api/packages', requireAuth, wrap(async (req, res) => {
  const { tab, q, invoice, customer, toko, courier, code, status, pickup_type, date, pickup_code } = req.query;
  const cond = [];
  const values = [];
  ...
  // SKIP Meilisearch jika pickup_code filter aktif — Meilisearch tidak bisa
  // filter kombinasi pickup_code + search per kolom dengan andal, pakai SQL.
  if (hasSearchText && req.query.kanban !== '1' && !pickup_code) {
    // Build Meilisearch search query
    let searchQuery = '';
    if (q && String(q).trim()) {
      searchQuery = String(q).trim();
    } else {
      // Search per kolom: query difilter per field (akurat), bukan digabung jadi
      // satu string yang ambigu. Search query kosong → Meilisearch pakai filter saja.
      searchQuery = '';
    }
    ...
    if (invoice && String(invoice).trim()) meiliFilters.invoice = String(invoice).trim();
    if (customer && String(customer).trim()) meiliFilters.customer = String(customer).trim();
    if (toko && String(toko).trim()) meiliFilters.toko = String(toko).trim();
    if (courier && String(courier).trim()) meiliFilters.courier = String(courier).trim();
    if (code && String(code).trim()) meiliFilters.code = String(code).trim();
    ...
    if (pickup_code === 'empty' || pickup_code === 'filled') {
      meiliFilters.pickup_code = pickup_code;
    }
  ...
  if (tab === 'arsip') {
    ...
    // Kanban hanya menampilkan 9 status pipeline (kolom di KanbanScreen).
    // Tanpa batasan ini, data_masuk (10rb+) & absen_ambil_customer ikut
    // terambil walau tidak dirender, mendorong paket lain keluar dari
    // LIMIT 2000 sehingga kolom terlihat kosong saat filter "Semua".
    values.push(KANBAN_STATUSES);
  ...
  // Filter status generate pickup code (tab selfpickup):
...
app.post('/api/packages/archive', requireAuth, requireRole('superadmin', 'admin'), wrap(async (req, res) => {
  ...
  if (r.rowCount > 0) {
    // Re-index paket yang diarsip supaya hasil search (tab aktif) tidak lagi memuatnya.
    const arch = await pool.query(
      `SELECT ${PACKAGE_LIST_COLUMNS} FROM packages WHERE id = ANY($1::int[])`,
      [r.rows.map((x) => x.id)]
    );
    bulkIndexPackages(arch.rows);
  }
  notify();
...
app.post('/api/packages/find-by-code', requireAuth, requireRole('admin', 'sales', 'warehouse', 'superadmin'), wrap(async (req, res) => {
...
// Idempoten: kalau paket sudah punya pickup_code, kembalikan kode yang ada
// (TIDAK generate ulang) — melindungi dari klik berulang yang bikin kode
// berubah-ubah di sisi mana pun (frontend, retry, dll).
app.post('/api/packages/:id/pickup-code', requireAuth, requireRole('sales', 'superadmin'), wrap(async (req, res) => {
  ...
  if (!existing.rows[0]) return res.status(404).json({ error: 'Paket tidak ditemukan' });
  if (existing.rows[0].pickup_code) {
    // Sudah punya kode → kembalikan apa adanya, jangan ubah.
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      const r = await pool.query(
        `UPDATE packages SET pickup_code=$2, updated_at=now() WHERE id=$1 AND (pickup_code IS NULL OR pickup_code='') RETURNING *`, [id, code]);
      if (!r.rows[0]) {
        // Race: sudah ada kode dari percobaan lain → ambil yang sudah tersimpan.
        const now = await pool.query('SELECT * FROM packages WHERE id=$1', [id]);
        return res.json(now.rows[0] || existing.rows[0]);
      }
      ...
      try {
        ...
        if (!ex) {
          ...
          if (hasChange) {
            ...
            const upd = await pool.query(`UPDATE packages SET ${updates.join(', ')}, updated_at=now() WHERE id=$1 RETURNING *`, vals);
            updated++;
            if (upd.rows[0]) indexPackage(upd.rows[0]);
          } else {
            skipped++;
          }
        }
